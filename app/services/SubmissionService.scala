...
import models.{ReadSubmissionRequest, ReadSubmissionRequestCommon, ReadSubmissionRequestDetails, RequestSubmissionHistoryParameters, SubmissionsListRequest, SubmittedReport, UserAnswers}
...
class SubmissionService @Inject() (submissionsConnector: SubmissionsConnector, repository: SubmissionsRepository)(implicit ec: ExecutionContext) {
  ...
  def readAndMaybeCache(requestBody: RequestSubmissionHistoryParameters)(implicit hc: HeaderCarrier): Future[List[SubmittedReport]] = {
    val submissionRequest = ReadSubmissionRequest(
      ...
      for
        ...
        submissions = submissionResponse.submissionsListResponse.responseDetails.submissionsList
        //NOTE: we should wipe their data and replace with new record if cache option is true
        _ <- if (requestBody.shouldCache) repository.set(UserAnswers(requestBody.userId, Json.toJson(submissions).as[JsObject])) else Future.successful(Done)
      yield submissions
    } 
