...
class SubmissionsConnector @Inject() (val config: AppConfig, val repository: SubmissionsRepository, val http: HttpClientV2)(implicit ec: ExecutionContext)
    extends Logging {
    ...
    http
      ...
      .flatMap {
        case res if res.status == 200 =>
          res.json.validate[ReadSubmissionResponse] match {
            case JsSuccess(submissionHistory, _) => Future.successful(submissionHistory)
            case JsError(errors) =>
              logger.error(s"Invalid json returned; $errors for corr ID : ${correlationID.toString}")
